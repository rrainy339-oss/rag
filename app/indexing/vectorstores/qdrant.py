...
class QdrantVectorStore:
    ...
    Sparse/BM25 retrieval and RRF fusion are handled by the retrieval module.
    This adapter writes dense vectors plus payloads with stable point IDs, and
    supports both Qdrant server URLs and local persistent Qdrant storage paths.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if url and path:
            raise ValueError("QdrantVectorStore accepts either url or path, not both.")

        try:
            from qdrant_client import QdrantClient, models
        except ImportError as exc:
            raise VectorStoreDependencyError(
                "QdrantVectorStore requires qdrant-client. Install the indexing "
        ...
        if self.client.collection_exists(self.collection_name):
            ...
            if actual_size is not None and actual_size != self.vector_size:
                raise ValueError(
                    ...
                    f"{actual_size}, expected {self.vector_size}."
                )
            return
